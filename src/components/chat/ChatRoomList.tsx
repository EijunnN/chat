...
export function ChatRoomList() {
  ...
  const activeTab = (searchParams.get("tab") as "my" | "public") || "my";
  ...
  const handleChatCreated = (chatRoom: InstanceOfSchema<typeof ChatRoom>) => {
    router.push(`/chat/${chatRoom.$jazz.id}?tab=${activeTab}`);
  };

  if (!profile) return null;

  const myRooms =
    profile.chatRooms?.filter(
        ...
        <div className="p-6 pb-2 border-b-2 border-foreground">
          ...
          <div className="flex gap-4 border-b border-border pb-px">
            <button
              onClick={() =>
                router.push(
                  `/chat${currentRoomId ? `/${currentRoomId}` : ""}?tab=my`,
                )
              }
            ...
            <button
              onClick={() =>
                router.push(
                  `/chat${currentRoomId ? `/${currentRoomId}` : ""}?tab=public`,
                )
              }
              className={`text-xs font-sans uppercase tracking-widest pb-2 -mb-[1px] transition-colors ${
                activeTab === "public"
              ...
              {roomsToShow.map((room) => {
                ...
                return (
                  <li key={room.$jazz.id}>
                    <button
                      onClick={() =>
                        router.push(`/chat/${room.$jazz.id}?tab=${activeTab}`)
                      }
                      className={`w-full text-left group`}
                    >
                      <div
                        className={`flex items-baseline justify-between border-b ${isActive ? "border-foreground" : "border-border group-hover:border-foreground"} pb-1 transition-colors`}
                      >
                        <span
                          className={`font-serif font-bold text-lg ${isActive ? "text-foreground" : "text-foreground/80"}`}
                        >
                          {room.name}
                        </span>
                        {room.isPublic ? (
